...
# --- Đồng bộ menu với session ---
if menu != st.session_state.page and st.session_state.page != "Thanh toán":
...
# ==============================
    ...
    if not st.session_state.payment_success:
        ...
        if st.button("⬅️ Quay lại giỏ hàng", use_container_width=True):
            st.session_state.page = "Giỏ hàng"
            st.rerun()

    else:
        st.success("🎉 Thanh toán thành công! Cảm ơn bạn đã mua hàng.")
        if st.button("🛒 Giỏ hàng", use_container_width=True):
            st.session_state.payment_success = False
            st.session_state.page = "Giỏ hàng"
            st.rerun()
