...
def load_data_from_drive(sample_size=5000):
    ...
    # Kiểm tra vài ký tự đầu để đoán format
...
def get_top_n_knn(seed_item, top_n=10):
    """
    Gợi ý sản phẩm tương tự (KNN) dựa trên model đã train sẵn.
    Fix đúng chiều dữ liệu (n_features = 80112).
    """
    ...
    if hasattr(knn_model, "_fit_X"):
        ...
        if item_idx >= fit_X.shape[0]:
            return []
        # ✅ Lấy vector hàng (đúng chiều: 1 x 80112)
    ...
    if hasattr(knn_model, "_fit_X"):
        for idx in indices:
            if idx < len(i_idx2id):
                similar_items.append(i_idx2id[idx])
