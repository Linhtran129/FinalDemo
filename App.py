...
# ==============================
# ⚙️ 1️⃣ Load model trước khi xử lý dữ liệu
# ==============================
@st.cache_resource
def load_model():
...
def get_top_n_knn(seed_item, top_n=10):
    ...
    # lấy ra n_neighbors, có thể nhỏ hơn top_n nếu model hạn chế
...
if len(st.session_state.cart) == 0:
    ...
    if st.sidebar.button("💳 Thanh toán", key="checkout"):
        st.sidebar.success("✅ Thanh toán thành công!")
        st.session_state.cart = []
        st.rerun()  # reset giao diện
